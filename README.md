...
[![Version](https://img.shields.io/badge/version-7.3.19-green)](https://github.com/gr1ng0333/veles/releases)
...
**Версия:** 7.3.19 | **Репозиторий:** [github.com/gr1ng0333/veles](https://github.com/gr1ng0333/veles) | **Ветка:** `veles`

---

## Release notes
### v7.3.19
- 7.3.19 — removed hardcoded deployment secrets from the tracked legacy VPS deploy helper, stopped Colab bootstrap from embedding GitHub tokens in remote URLs, and added a tracked-secret guard test for Telegram, GitHub, OpenAI-style keys, JWT blobs, private keys, cloud keys, and credentialed URLs

### v7.3.18
- 7.3.18 — fixed stale context tails: old progress/tools/events/supervisor fallbacks and task reflections are now age-filtered, so archived April diagnostics no longer masquerade as current runtime context after quiet periods

### v7.3.17
- 7.3.17 — fixed codex env resync so `sync_mode_env_from_state()` no longer overwrites `OUROBOROS_MODEL` back to `codex/gpt-5.4`; active codex tasks now honor `OUROBOROS_MODEL_CODE` / `.env` overrides after restart
...
### v7.3.13
- 7.3.13 — fixed plan contour: draft plan now visible in context across model switches (Opus→Codex), plan_approve auto-detects latest draft without requiring plan_id, step_done auto-completes plan on last step, plan_complete is idempotent

### v7.3.12
- 7.3.12 — expanded remote operator docs and SSH/3x-ui skills for fleet-scale registry discipline, native panel monitoring, and standalone `fleet_monitor.py` operations

### v7.3.11
- 7.3.11 — added standalone `scripts/fleet_monitor.py` for cron/systemd timer usage with JSON snapshot/history output, fleet verdict exit codes, and coverage for direct script execution

### v7.3.10
- 7.3.10 — added `fleet_health`: parallel fleet aggregation for SSH health, Xray diagnostics, and 3x-ui panel monitoring with tag/alias filtering and per-server ok/warn/critical verdicts

### v7.3.9
- 7.3.9 — added native 3x-ui HTTP API monitoring: `xui_panel_status`, panel credentials in SSH target registry, and operator visibility for whether panel auth is configured
...
### v7.3.6
- 7.3.6 — incoming archive routing now accepts `.zip/.rar/.7z/.tar*` as real inbox artifacts, raises archive download limit to 100 MB, exposes captioned archives as payloads, and reports true download failures instead of falsely claiming success

### v7.3.5
- 7.3.5 — `pdf_variant_bot` step 5: add CLI inspection/validation commands (`inspect-db`, `inspect-set`, `list-issues`, `validate-set`) plus reporting module and coverage for import-quality checks

### v7.3.4
- 7.3.4 — `pdf_variant_bot` step 4: parse answers PDFs, match answers to parsed tasks by `block_code` + `task_number`, store unmatched/ambiguous cases in `import_issues` without guessing

### v7.3.3
- 7.3.3 — `pdf_variant_bot` step 3: parse task PDFs into blocks/tasks/assets with `markitdown` + `PyMuPDF` integration, CLI `parse-tasks`, issue logging for ambiguous asset mapping

### v7.3.2
- 7.3.2 — `pdf_variant_bot` step 2: archive ingest CLI, deterministic unpack storage, source file registry, SHA-256 dedupe, import run metadata

### v7.3.1
- 7.3.1 — scaffold `pdf_variant_bot`: normalized SQLite schema + `init-db` CLI for upcoming PDF/import pipeline

### v7.3.0
- 7.3.0 — Focused workers: isolated lightweight agents for external projects; tool whitelist (DEPLOY/CODE/FULL/custom), minimal context (no identity/scratchpad/chat history), mandatory TG notification on completion; `create_focused_worker` tool + event handler

### v7.2.1
- 7.2.1 — Fix send_with_budget/log_chat callers after API refactor (crash loop fix); systemd unit for process management (replaces screen+watchdog cron)

### v7.2.0
- 7.2.0 — Single-file Codex token storage (`codex_accounts.json`): atomic writes survive crashes, .env only as initial seed, lazy refresh (never burns tokens on restart), `refresh_all_quotas()` quota-only probe; cron watchdog for process recovery

### v7.1.58
- 7.1.58 — auto-route images through Codex VLM: owner photos & browser screenshots auto-described via codex/gpt-5.4-mini before entering main LLM; antiword for legacy .doc parsing in RAG
...
- 7.1.26 — новый инструмент `self_audit`: prescriptive codebase и operational health audit — ranked actionable findings: oversized modules, import errors, recurring timeouts, evolution no-commit patterns, pattern register hits.
...
- 6.85.4 — Copilot 5xx в реальном owner-task больше не уводят раунд в немедленный fallback: текущий раунд логируется, аккаунт ставится на 60-секундный cooldown и тот же самый раунд автоматически повторяется через минуту в рамках того же `task_id`.
...
### v7.1.55 (2026-04-05)
- rollback to v7.1.55: removed 86 unstable evolution commits (v7.1.56–v7.1.86), reduced MAX_SESSION_RESETS 10→5
...
- fix: evolution round limit raised to 280 for Copilot contour
- fix: pyproject.toml version sync
...
### v7.1.48 (2026-04-04)
- migrate aux tools to Codex/Copilot: research_report, web_monitor, search use get_aux_light_model(); SYSTEM.md reviewer guidance updated to get_evolution_reviewer_models()
- model_modes: get_evolution_reviewer_models() returns [codex/gpt-5.4, copilot/claude-sonnet-4.6]; DEFAULT_EVOLUTION_REVIEWER_MODELS constant
- yt_reader: watchlist-based YouTube monitor (yt_subscribe/unsubscribe/status/check/latest); inbox: YouTube as 8th source
### v7.1.47 (2026-04-04)
- remote_network_diag: 7 network diagnostic tools (remote_ping/traceroute/port_check/dns_lookup/vpn_status/iptables_summary/netstat)
### v7.1.46 (2026-04-04)
...
### v7.1.43 (2026-04-04)
- reddit_reader: Reddit subreddit monitor with watchlist via public JSON API (6 tools)
- inbox: Reddit integrated as 5th source in inbox_check/inbox_status
...
### v7.1.40 (2026-04-04)
- rss_reader: RSS/Atom feed subscriptions with watermark-based new-item tracking (rss_subscribe/rss_check/rss_unsubscribe/rss_status)
...
### v7.1.29 (2026-04-04)
- Copilot Round Policy moved from context.py to SYSTEM.md: English, 28-round model, updated phases 1-8/9-19/20-27, session boundary at round 27

### v7.1.28 (2026-04-04)
- Session reset billing fix v2: trailing system message после session reset rebuild. Структура [system, user, system] → last_role="system" → initiator="agent" → не биллится.

### v7.1.26 (2026-04-03)
- `self_audit` tool: prescriptive codebase and operational health audit — ranked actionable findings (CRITICAL/HIGH/MEDIUM) covering: import errors, oversized modules (BIBLE P5), oversized functions, missing tests, recurring tool timeouts, no-commit evolution patterns, high-count pattern register items
...
### v7.1.22 (2026-04-03)
- `memory_search` tool: TF-IDF search across chat history, knowledge base, identity.md, scratchpad — recall past decisions and patterns
- `copilot_proxy.py` refactor: `call_copilot` decomposed into `_dump_diag_payload`, `_merge_choices`, `_execute_single_account` — fits 220-line limit

### v7.1.20
- Diagnostic payload logging: временный дамп первого Copilot payload по interaction_id в `/opt/veles-data/logs/copilot_diag.jsonl` для исследования биллинга.

### v7.1.19
- Consciousness audit fixes: context.py threshold mismatch (100→500), browser_recovery.py unsafe page calls, __init__.py broken encoding fallback; dead code cleanup (codex_proxy_format, compact_context); git_history.py duplicate SHA field removed.

### v7.1.18
- Session reset billing fix: убран assistant message из post-reset messages. Структура после reset теперь [system, user] — идентична первому запросу задачи. GitHub не считает premium request.
...
### 6.84.3
- Copilot evolution rollover: after every 30 rounds start a new interaction_id and force the first request of the new thread to be user-initiated, so premium rescue happens before the usual 33–35 round fallback cliff.

### v6.84.3 (2026-03-28)
- TikTok tools: `tiktok_search`, `tiktok_metadata`, `tiktok_profile`, `tiktok_history` — search/discover TikTok content without video download; dedup history registry.
...
### v6.83.9 (2026-03-28)
- `code_search` tool: structured codebase search (literal/regex/symbol modes), file/ext/path filters, context lines, cross-file references — replaces fragile `run_shell grep` for code analysis.

### v6.83.8 (2026-03-28)
-  tool: structured JSONL log querying (filter by type/task_id/time/field, aggregation, field extraction).

### v6.83.7
- Supervisor commands `/status`, `/review`, `/evolve`, `/bg` are now terminal and no longer leak into direct chat.
- Evolution enqueue now skips while direct chat is active, preventing dual-path Copilot evolution startup.

### v6.83.6
- Evolution policy now distinguishes Growth vs Hygiene, forces Growth under green invariants, prioritizes new tools, and warns against hygiene-only drift after a hygiene cycle.
- Evolution completion now records the latest commit kind (`growth` / `maintenance` / `hygiene`) so the next cycle sees whether it is drifting into release tails.
- Copilot-only evolve pacing: add a 4s delay before each post-first Copilot LLM request (including fallback hops) without affecting Codex or normal chat.

### v6.83.5
- Fix run_shell crash on Python-list-style string commands (e.g. "[grep, -n, ...]").
- Add test_shell_cmd_recovery: regression tests for malformed cmd detection.
- Resync pyproject.toml and README to VERSION 6.83.5.

### v6.83.4
- Reflection fallback chain: survive codex/gpt-5.4-mini 401 via qwen/qwen3-coder:free; fixes all silent reflection failures.
- Resync release metadata: VERSION, pyproject.toml, README all on 6.83.4.

### v6.83.3
- Fixed `NameError: ctx is not defined` in `_emit_task_results` by passing `ctx` from `handle_task`, restoring post-task result emission.
- Re-synchronized release metadata after the hotfix so `VERSION`, `pyproject.toml`, README markers, and the latest release tag point to the same 6.83.3 line again.

### v6.83.2
- Copilot-only persistence hardening: single-writer lock for mutating write paths, rescue snapshots pushed to `refs/veles-rescue/*`, and truthful `rescued` / `interrupted_before_push` completion semantics before reset/finalize.
...
### 6.82.7
- Hotfix: `call_copilot()` now accepts `tool_choice` and other kwargs expected by `LLMClient.chat()`, and forwards `tool_choice` into the Copilot Chat Completions payload.

### 6.82.4
- Hardcoded `reasoning_effort="high"` for every Copilot-routed Claude request, regardless of caller-provided effort settings.
- Left Codex and OpenRouter reasoning behavior unchanged; only the Copilot transport now enforces the high-reasoning invariant.

### 6.82.3
- Ported selected last30days-inspired query signals into the native search stack: query typing, core-subject extraction, recency markers, overlap-based relevance, and stable dedupe signatures.
- Propagated the new signals through query planning, ranking, and research-run traces so recency-aware behavior is visible in both `query_plan` and `candidate_sources`.
- Added targeted coverage for docs/benchmark query typing, recency scoring, duplicate handling, and kept the structured search/research/smoke contour green.

### 6.82.2
- Split the structured search contour so `ouroboros/tools/search.py` becomes a thinner facade while page reading and synthesis move into `search_reading.py` and `search_synthesis.py`.
- Preserved the public/test patch surface in `ouroboros.tools.search` through compatibility wrappers, so the refactor stays behavior-preserving instead of breaking monkeypatch-based contracts.
- Kept targeted search/research/smoke coverage green after the split, preparing the stack for the next recency-aware last30days port step.
...
- Covered plan progress emission with targeted tests for create/approve/update/step_done/complete/reject flows.

### 6.81.0
- Added transport-aware tool result caps with stricter Codex truncation for large tool outputs so long tool loops stop inflating later rounds as aggressively.
- Made compaction earlier and more aggressive for Codex tasks, reducing prompt growth across long-running multi-round executions.
- Tightened protected-tool-aware context compaction so compressed history stays useful while consuming fewer prompt tokens.
...
- Kept the public planning contract intact by re-exporting intent policies and query-plan builder through `ouroboros.tools.search`.
- Brought `ouroboros/tools/search.py` back under the 1000-line smoke ceiling while keeping search/research tests green.
...
- Closed the release-contour gap where code had already moved past `6.71.9` but README badges/text and tags still pointed to the older release line.

### 6.71.9
- Made `/accounts` human-readable again for degraded Codex accounts: each entry now shows an explicit diagnostics line with `last_error.reason`, category, and HTTP status instead of hiding the cause behind quota dashes.
- Added quota hints to the display layer (`5h/7d used` and plan when present), so exhausted windows and partial quota state are visible directly in the operator-facing output.
- Split display diagnoses into `auth_failure`, `temporary 429`, and `usage_limit_reached` without changing account rotation policy, and covered the formatter with regression tests.

### 6.71.8
- Codex account rotation now classifies `429` separately from auth failures: `usage_limit_reached` / rate-limit no longer falls into the same recovery path as `401`/`403`.
- Added explicit per-account last-error telemetry and `/accounts`-compatible status aliases so rate-limit reason, HTTP code, and quota-derived hints remain visible for accounts like #3/#4.
- Added stronger backoff behavior for exhausted quota windows and regression tests for Codex HTTP failure classification and account status exposure.

### 6.71.4
- Raised smoke function-count ceiling from 1000 to 1050 to match current codebase size; full smoke green again.
