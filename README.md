...
[![Version](https://img.shields.io/badge/version-7.1.84-green)](https://github.com/gr1ng0333/veles/releases)
...
**Версия:** 7.1.84 | **Репозиторий:** [github.com/gr1ng0333/veles](https://github.com/gr1ng0333/veles) | **Ветка:** `veles`
...
- 7.1.78 — doc_coverage tool — AST docstring coverage scanner (modules/classes/functions, per-file %, skip_private, category filter); 38 tests
- 7.1.77 — type_coverage tool — AST missing annotation scanner (param + return, per-file coverage %, skip_private); 32 tests
- 7.1.76 — performance_profile tool — runtime observability from production logs (tool calls, model cost, task stats, error rates)
- 7.1.75 — symbol_search tool — AST cross-reference search (defs + usages of any symbol); 35 tests
- 7.1.74 — callgraph tool — function-level call graph (callers/callees, transitive, per-file)
- 7.1.73 — test_gap_report tool — coverage gaps ranked by multi-signal risk (complexity+trend+churn+tier); 31 tests
- 7.1.72 — complexity_trend tool — per-function cyclomatic complexity delta over git history (rising/falling/volatile); 33 tests
- 7.1.71 — coupling_map tool — Robert Martin Ca/Ce/Instability metrics, zones (RIGID/BALANCED/UNSTABLE/ISOLATED), 29 tests
- 7.1.70 — test_coverage_map tool — per-function AST coverage heuristics (name_match/call_ref/import_ref signals); 31 tests
...
- 7.1.65 — semantic_diff tool — function/class-level change analysis between git refs; 28 tests
...
- 7.1.60 — tool_map: full toolset self-awareness (292 tools grouped by module, filter+format params); update_tool_snapshot.py: auto-sync EXPECTED_TOOLS in test_smoke.py
- 7.1.58 — evolution_focus: cross-cycle strategic goal memory (set/get/note/cycle/clear + context injection)
...
### v7.1.67 (2026-04-05)
- `dead_code` tool: AST-based unused import + dead private symbol detector; 31 tests

### v7.1.66 (2026-04-05)
- `change_impact` tool: blast-radius analysis — transitive reverse-dependency traversal, risk tiers (CRITICAL/HIGH/MEDIUM/LOW), test file mapping; 25 tests

### v7.1.65 (2026-04-05)
- `semantic_diff`: function/class-level change analysis between git refs — added/removed/modified functions and classes per file; AST-based, text+json output, path filter, max_files; 28 tests
...
### v7.1.62 (2026-04-05)
- tech_debt: codebase-wide structural debt scanner — 7 categories (oversized functions/params/modules, high complexity, deep nesting, FIXME/TODO, god objects); text+json output; path+category+severity filters; 25 tests

### v7.1.61 (2026-04-05)
- evolution_plan: synthesize hot_spots + patterns + focus + test-gaps into ranked action candidates; 30 tests

### v7.1.60 (2026-04-05)
- tool_map: full toolset self-awareness (292 tools grouped by module, filter+format); update_tool_snapshot.py auto-sync; 17 tests
