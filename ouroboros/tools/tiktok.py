...
def _tiktok_search(ctx: ToolContext, query: str, max_results: int = 10) -> str:
    ...
    cmd = [
        *ytdlp, "--no-download", "--flat-playlist", "--dump-single-json",
        "--no-warnings", "--extractor-args", "tiktok:webpage_download=0",
        search_target,
    ]
    try:
    ...
    return json.dumps({"status": "ok", "query": query, "count": len(results), "results": results},
...
def _tiktok_metadata(ctx: ToolContext, url: str) -> str:
    ...
    try:
        ...
        return json.dumps({"status": "failed", "error": str(exc)})
    cmd = [
        *ytdlp, "--no-download", "--dump-single-json", "--no-warnings",
        "--extractor-args", "tiktok:webpage_download=0", url,
    ]
    try:
...
def _tiktok_profile(ctx: ToolContext, username: str, max_results: int = 20) -> str:
    ...
    cmd = [
        *ytdlp, "--no-download", "--flat-playlist", "--dump-single-json",
        "--no-warnings", "--playlist-end", str(max_results),
        "--extractor-args", "tiktok:webpage_download=0", profile_url,
    ]
    try:
        proc = _run_ytdlp(cmd, timeout=45)
    except subprocess.TimeoutExpired:
    ...
    return json.dumps({
        "status": "ok", "username": username, "profile_url": profile_url,
...
def _load_history(ctx: ToolContext) -> dict:
...
def _save_history(ctx: ToolContext, data: dict) -> None:
    path = _history_path(ctx)
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")


def _tiktok_history(ctx: ToolContext, action: str = "list", url: str = "",
                    title: str = "", note: str = "") -> str:
    action = str(action or "list").strip().lower()
    url = str(url or "").strip()
    if action not in ("list", "add", "check", "clear"):
        return json.dumps({"status": "failed", "error": f"unknown action {action!r}"})
    data = _load_history(ctx)
    urls_set = set(data.get("urls") or [])
    if action == "list":
        return json.dumps({"status": "ok", "count": len(urls_set),
                           "urls": sorted(urls_set), "entries": data.get("entries") or []},
                          ensure_ascii=False, indent=2)
    if action == "clear":
    ...
    if not already_present:
        urls_set.add(url)
        entry = {"url": url, "title": str(title or "").strip(),
                 "note": str(note or "").strip(),
                 "added_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}
        entries = list(data.get("entries") or [])
        entries.append(entry)
        _save_history(ctx, {"urls": sorted(urls_set), "entries": entries})
    return json.dumps({"status": "ok", "url": url, "already_present": already_present,
                       "total": len(urls_set)}, ensure_ascii=False, indent=2)
    ...
    return [
        ToolEntry(
            ...
            schema={
                ...
                "description": (
                    ...
                    "No video download. Use instead of web_search for TikTok discovery."
                ),
                "parameters": {
                    "type": "object",
                    "properties": {
                        "query": {"type": "string",
                                  "description": "Search query. Examples: 'python tutorial', '#ai', '@mkbhd'"},
        ...
        ToolEntry(
            ...
            schema={
                ...
                "description": (
                    ...
                    "Returns: title, description, uploader, duration, view_count, like_count, "
                    "comment_count, upload_date, tags, thumbnail URL. No video download."
                ),
                "parameters": {
                    "type": "object",
                    "properties": {
        ...
        ToolEntry(
            ...
            schema={
                ...
                "description": (
                    ...
                    "Returns list: title, url, duration, view_count, like_count, upload_date. "
                ...
                "parameters": {
                    ...
                    "properties": {
                        "username": {"type": "string",
                                     "description": "TikTok username e.g. 'mkbhd' or '@mkbhd'"},
                        "max_results": {"type": "integer",
                                        "description": "Max videos (1-100, default 20)"},
                    },
                    "required": ["username"],
                },
            },
        ...
        ToolEntry(
            ...
            schema={
                "name": "tiktok_history",
                "description": (
                    "Manage TikTok sent-video dedup registry in drive memory. "
                    "action='list' returns all URLs; 'add' records a URL; "
                    "'check' tests if URL is present; 'clear' wipes history."
                ),
                "parameters": {
                    "type": "object",
                    "properties": {
                        ...
                        "url": {"type": "string",
                               "description": "TikTok URL (required for add/check)"},
