...
Provides model pricing lookup (static + lazy OpenRouter sync),
...
# Prices per 1M tokens: (input, cached, output)
# From OpenRouter API (2026-03-19). Update periodically.
MODEL_PRICING_STATIC: Dict[str, Tuple[float, float, float]] = {
    ...
    "anthropic/claude-haiku-4.5": (0.80, 0.080, 4.0),
    ...
    "openai/gpt-5.4-mini": (0.30, 0.030, 1.20),
    ...
    # Codex shadow pricing (free via OAuth, tracked for budgeting)
    "gpt-5.4": (1.75, 0.175, 14.0),
    "gpt-5.4-mini": (0.30, 0.030, 1.20),
    "gpt-5.3-codex": (1.75, 0.175, 14.0),
    # Copilot shadow pricing (free via GitHub)
    "claude-haiku-4.5": (0.80, 0.080, 4.0),
...
def get_pricing() -> Dict[str, Tuple[float, float, float]]:
    ...
    Falls back to static pricing if fetch fails. Thread-safe.
    """
...
def estimate_cost(
    ...
    """Estimate cost in USD for a single LLM call. Returns 0 if model unknown."""
