...
from supervisor.doc_payload import document_to_text_payload as _document_to_text_payload
install_apply_patch()
...
# debug: digest scheduler disabled for Codex debug
# _digest_scheduler.start()
log.info("DigestScheduler daemon DISABLED (debug)")
def _apply_background_runtime_policy() -> None:
    st = load_state()
    # debug: all background threads disabled for Codex debug
...
while True:
    ...
    for upd in updates:
        ...
        if msg.get("photo"):
            ...
            doc = msg["document"]
            text_override, doc_image_data, handled = _document_to_text_payload(doc, caption, TG, chat_id, DRIVE_ROOT, send_with_budget, int(msg.get("message_id") or 0))
            if not handled:
                continue
            if text_override:
                text = text_override
                    ...
                    if _uid2 and _batch_state.get("owner_id") and _uid2 == int(_batch_state["owner_id"]):
                        ...
                        if not _batched_image:
                            ...
                            if _fid2:
                                ...
                                _doc_text2, _doc_img2, _doc_handled2 = _document_to_text_payload(_msg2.get("document") or {}, _txt2, TG, _cid2, DRIVE_ROOT, send_with_budget, int(_msg2.get("message_id") or 0))
                                if _doc_text2:
