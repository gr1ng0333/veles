...
def test_project_init_python_creates_repo_and_first_commit(tmp_path):
    payload = json.loads(_project_init(_ctx(tmp_path), name='Demo API', language='python', description='Small python service'))
...
def test_project_file_read_reads_utf8_content_inside_local_project(tmp_path):
    ...
    _project_file_write(_ctx(tmp_path), name='demo-api', path='src/demo_api/config.json', content='{"port": 8080}\n')
    payload = json.loads(_project_file_read(_ctx(tmp_path), name='demo-api', path='src/demo_api/config.json'))
    assert payload['status'] == 'ok'
...
def test_project_file_read_truncates_when_max_chars_is_small(tmp_path):
    _project_init(_ctx(tmp_path), name='Demo API', language='python')
    _project_file_write(_ctx(tmp_path), name='demo-api', path='README.md', content='abcdefghij')
    payload = json.loads(_project_file_read(_ctx(tmp_path), name='demo-api', path='README.md', max_chars=5))
    assert payload['status'] == 'ok'
...
def test_project_file_read_truncates_with_marker_for_normal_limits(tmp_path):
    _project_init(_ctx(tmp_path), name='Demo API', language='python')
    _project_file_write(_ctx(tmp_path), name='demo-api', path='README.md', content='abcdefghijklmnopqrstuvwxyz')
    payload = json.loads(_project_file_read(_ctx(tmp_path), name='demo-api', path='README.md', max_chars=20))
    assert payload['status'] == 'ok'
    assert payload['file']['truncated'] is True
...
def test_project_file_write_updates_file_inside_local_project(tmp_path):
    _project_init(_ctx(tmp_path), name='Demo API', language='python')
    payload = json.loads(_project_file_write(_ctx(tmp_path), name='demo-api', path='src/demo_api/config.json', content='{"port": 8080}\n'))
...
def test_project_github_create_attaches_origin_and_reports_slug(tmp_path, monkeypatch):
    ...
    payload = json.loads(_project_github_create(_ctx(tmp_path), name='demo-api', owner='veles', private=False, description='Demo repo'))
    repo_path = pathlib.Path(payload['project']['path'])
    assert payload['status'] == 'ok'
...
def test_project_push_pushes_current_branch_to_origin(tmp_path):
    ...
    remote_head = subprocess.run(['git', f'--git-dir={remote_dir}', 'rev-parse', 'refs/heads/main'], check=True, capture_output=True, text=True).stdout.strip()
    assert payload['status'] == 'ok'
...
def test_project_status_reports_clean_repo_and_latest_commit(tmp_path):
    ...
    assert payload['remote_awareness'] == {'available': False, 'reason': 'origin remote not configured'}

def test_project_status_reports_untracked_and_remote_entries(tmp_path):
    _project_init(_ctx(tmp_path), name='Demo API', language='python')
    _project_file_write(_ctx(tmp_path), name='demo-api', path='notes/todo.txt', content='ship it\n')
    repo_dir = pathlib.Path(_ctx(tmp_path).drive_root) / 'projects' / 'demo-api'
    remote_dir = tmp_path / 'remote.git'
    subprocess.run(['git', 'init', '--bare', str(remote_dir)], check=True)
    subprocess.run(['git', 'remote', 'add', 'origin', str(remote_dir)], cwd=repo_dir, check=True)
    ...
    assert payload['working_tree']['changes'] == [{'path': 'notes/todo.txt', 'index': '?', 'worktree': '?', 'kind': 'untracked'}]
    assert payload['remotes'] == [{'name': 'origin', 'url': str(remote_dir), 'direction': 'fetch'}, {'name': 'origin', 'url': str(remote_dir), 'direction': 'push'}]
